...
vector<string> getSubstrings(string word) {
    ...
    {
        for (int j = i+1; j < int(word.size()+1); j++)
        {
            if (i == 0 && j == int(word.size())) continue;
 ...
 * */
void printOutput(vector<string>& words, unordered_map<string, unordered_set<string>>& words_map, unordered_map<string, int> duplicates) {
    cout << "output:" << endl;
    for (auto &&word : words)
    {
        cout << "\t" << word << ": ";
...
void containsSubstrings(vector<string>& words) {
    unordered_map<string, unordered_set<string>> words_map;
    ...
    for (auto &&word : words)
        for (auto &&sub_string : getSubstrings(word))
            if (words_map.find(sub_string) != words_map.end())
                words_map[sub_string].insert(word);
